...
class AzulPluginEntryPointCheck(BinaryPlugin):
    ...
    FEATURES = [Feature(name="tag", desc="Tags generated by PE entrypoint check", type=FeatureType.String)]
    ...
    def execute(self, job: Job):
        ...
        try:
            ...
            sections: list[pefile.SectionStructure] = pe.sections
        ...
        eps = sections[0]
        last_physical = sections[0]
        last_virtual = sections[0]
        entry_point_section_names = [b".text", b"INIT", b"UPX1"]

        for s in sections:
            # Find which section AddressOfEntryPoint points at
            if None in [s.VirtualAddress, s.PointerToRawData, s.SizeOfRawData]:
                raise AttributeError("Expected s.VirtualAddress and s.PointerToRawData ints, got None")
            if ep >= s.VirtualAddress:
                eps = s

            # Find the last section in memory
            if last_virtual.VirtualAddress is None:
                raise AttributeError("Expected last_virtual.VirtualAddress to be an int, got None")
            if last_virtual.VirtualAddress < s.VirtualAddress:
                last_virtual = s

            # Find the last section on disk
            if last_physical.PointerToRawData is None:
                raise AttributeError("Expected last_physical.PointerToRawData to be an int, got None")
        ...
        if last_virtual.SizeOfRawData is None:
            raise AttributeError("Expected last_virtual.SizeOfRawData to be an int, got None")
