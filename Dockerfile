...
RUN pip install \
    ...
    azul-plugin-entrypointcheck==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-entrypointcheck==$(cd /tmp/src && python -m setuptools_scm);fi
